...
var ApiClient = dojo.declare('ApiClient', net.Client, {
   ...
   *
   * TODO(termie): clean up hubs and devices
   */
  onDisconnect: function () {
    this.emit('disconnected', this);
  },
  /**
   * Event handler for Hub events. Builds a HubProxy, notifies listeners.
   * 
   * Also makes sure that further events from the remote ApiServer are
   * forwarded to the HubProxy.
   *
   * @param {Object} hub A serialized Hub. {@link Hub#toDict}
   */
  onHubAdded: function (data) {
    var hub = new proxy.HubProxy(data.name, data.kind, this);
    ...
    this.hub = hub;
    //this.hub.on('event', this.bubble());
    this.emit('hubAdded', this.hub);
  },
  /**
  ...
  onMessage: function (message) {
    if (message.topic == 'hubAdded') {
      this.onHubAdded(message.data);
    }
